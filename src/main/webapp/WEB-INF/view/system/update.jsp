...
<form action="${basePath}permission/update" method="post">
    ...
    <p>
        权限名称：<input type="text" name="name" value="${domain.name}"/>
    </p>
    <p>
        状态：
        <input type="radio" name="available" value="1"
               <c:if test="${domain.available == 1}">checked="checked"</c:if> >可用
        <input type="radio" name="available" value="0"
               <c:if test="${domain.available == 0}">checked="checked"</c:if>>不可用
    <p>
    <p>
        资源路径：<input type="text" name="url" id="url" value="${domain.url}"/>
    <p>
    <p>
        权限字符串：<input type="text" name="permission" value="${domain.permission}"/>
    <p>
    <p>
        模块ID：
